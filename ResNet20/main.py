...
def fault_aware_train_validation(train_loader, valid_loader, test_loader, device, args, round_num, pruning_rate=0.5,
    ...
    for ep in range(args.epochs):
        ...
        if ep >= args.start_pruning_epoch:
            current_pruning_rate = min(0.1 + ((ep - args.start_pruning_epoch) / (args.epochs - args.start_pruning_epoch)) * 0.4, pruning_rate)  # Start at 0.1 and increase to max 0.5
        else:
    ...
    with torch.no_grad():
        ...
        for ls, param, idx, layer_name, param_type, new_weight_value in loss_sorted:
            ...
            if isinstance(model_module, (torch.nn.Conv2d, torch.nn.Linear)) and param_type == 'weight':
                ...
                acc_temp = (100.0 * test_correct) / test_total
                if acc_temp < prev_acc:
                    prev_acc = acc_temp

                #acc_min_loss.append(prev_acc)
                acc_min_loss.append(acc_temp)
...
def pruning_model(test_loader, device, args):
    model = resnet20.ResNet20(num_classes=args.num_of_class).to(device)
    model.load_state_dict(torch.load(args.save_path + args.standard_model_name, map_location=torch.device('cpu')))
    ...
    prune.global_unstructured(
        ...
        amount=args.pruning_percentage
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--steps', type=int, default=200, help='Set the Steps of Training.')
    ...
    parser.add_argument('--pruned_model_name', type=str, default='Resnet20_pruned.pt',
                        help='The name of Original Model')
    parser.add_argument('--standard_noisy_model_name', type=str, default='Resnet20_Standard_with_Noise.pt',
                        help='The name of standard model with fault-aware training')
    ...
    parser.add_argument('--model', type=str, default='ResNet20',
                        help='The Model name')
    parser.add_argument('--pruning_percentage', type=float, default=0.5, help='The percentage of pruning')
    parser.add_argument('--prune_step', type=int, default=20,
                        help='The steps for doing pruning during the training')
    parser.add_argument('--start_pruning_epoch', type=int, default=50,
                        help='The steps for doing pruning during the training')
    ...
    for rnd in range(args.round_number):
        ...
        print(f'----------------------------------------------------------------------------------------')
        pruned_model, pruned_acc = pruning_model(test_loader, device, args)
        ...
        print(f'----------------------------------------------------------------------------------------')
        noisy_attack_time = attack(valid_loader, test_loader, device, args, rnd + args.start_point,
                                   mode='fault_aware', attack_type='sign', attack_scenario='important')
